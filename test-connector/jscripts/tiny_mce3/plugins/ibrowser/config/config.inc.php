<?php
	...
	$idBlog = intval(@$_REQUEST['idBlog']);
	preg_match('@[a-z0-9]+@', @$_REQUEST['canChangeImage'], $blcode);
	if ($blcode[0] == md5(serialize(array($idBlog, false)))) $canChangeImage = false;
	elseif ($blcode[0] == md5(serialize(array($idBlog, true)))) $canChangeImage = true;
	else exit();

	global $cfg;
	$cfg['lang']   		= 'vi';														// default language; e.g. 'en'		
	...
	$cfg['umax'] 		= 1;														// max. number of image files to be uploaded; default: 1; value > 1
  	$cfg['create'] 		= $canChangeImage; // allow to create directory: 'true' or 'false'
  	$cfg['delete'] 		= $canChangeImage; // allow deletion of image: 'true' or 'false'	
  	$cfg['rename'] 		= $canChangeImage; 
...
/*		array (
	...
	$cfg['ilibs_dir'] 	   = array('/uploads/blogs/'.($idBlog<758840? '': 'blogs2/').$idBlog.'/');						   	// image library path with slashes; absolute to root directory - please make sure that the directories have write permissions
